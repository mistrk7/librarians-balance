...
execute if score sealed_books libal.main matches 1 unless entity @s[nbt={SelectedItem:{tag:{sealed:1b}}}] if predicate libal:has_ench_book unless block ~ ~ ~ minecraft:lectern[has_book=true] run function libal:interact/judge/main with storage libal:jobs_find
