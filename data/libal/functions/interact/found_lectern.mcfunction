...
execute if score sealed_books libal.main matches 1 unless entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{sealed:1b}}}}] if predicate libal:has_ench_book unless block ~ ~ ~ minecraft:lectern[has_book=true] run function libal:interact/judge/ench_book with storage libal:books
execute unless score sealed_books libal.main matches 1 if predicate libal:has_ench_book unless block ~ ~ ~ minecraft:lectern[has_book=true] run function libal:interact/judge/ench_book with storage libal:books
