...
$execute as @e[type=villager,nbt={Brain:{memories:{"minecraft:job_site":{value:{pos:$(cord)}}}}}] run execute if data entity @s Offers.Recipes[0].buyB{Count:1b} run data modify storage libal:main slot set value 0
$execute as @e[type=villager,nbt={Brain:{memories:{"minecraft:job_site":{value:{pos:$(cord)}}}}}] run execute if data entity @s Offers.Recipes[1].buyB{Count:1b} run data modify storage libal:main slot set value 1
