...
$data modify entity @s ArmorItems[0].tag.Storage[0] set from entity @s Offers.Recipes[$(slot)].sell.tag
...
$data modify entity @s ArmorItems[0].tag.Storage[1].priceMultiplier set from entity @s Offers.Recipes[$(slot)].priceMultiplier
...
$execute store success score same_book libal.main run execute if data entity @s Offers.Recipes[$(slot)].buyB{Count:1b} run data modify entity @s Offers.Recipes[$(slot)].sell.tag set from entity @s ArmorItems[1].tag
$execute if data entity @s Offers.Recipes[$(slot)].buyB{Count:1b} run data modify entity @s Offers.Recipes[$(slot)].sell.tag merge value {traded:True}

#Set the trade to the books emerald value, unless the book already matches.
$execute unless score same_book libal.main matches 0 if data entity @s Offers.Recipes[$(slot)].buyB{Count:1b} run data modify entity @s Offers.Recipes[$(slot)].buy.Count set from storage libal:books cost_final
$execute unless score same_book libal.main matches 0 if data storage libal:books {cost_blocks:True} if data entity @s Offers.Recipes[$(slot)].buyB{Count:1b} run data modify entity @s Offers.Recipes[$(slot)].buy.id set value "minecraft:emerald_block"
#Set price multiplier to zero if it costs blocks
$execute unless score same_book libal.main matches 0 if data storage libal:books {cost_blocks:True} if data entity @s Offers.Recipes[$(slot)].buyB{Count:1b} run data modify entity @s Offers.Recipes[$(slot)].priceMultiplier set value 0
