...
execute if score cap_enchant_level libal.main matches 1 run execute as @e[type=villager,sort=nearest,limit=8,nbt={VillagerData:{profession:"minecraft:librarian"}}] run function libal:villager/trades/reduce with storage libal:books
